...
import { IProviderAccount, SessionEventTypes, SessionTypes, OptionalOperation } from '@multiversx/sdk-wallet-connect-provider/out';
...
import { defineCustomElements, PendingTransactionsModal, WalletConnectModal } from 'lib/sdkDappCoreUi';
import { logoutAction } from 'store/actions';
import { chainIdSelector, nativeAuthConfigSelector, walletConnectConfigSelector } from 'store/selectors';
import { getState } from 'store/store';
import { IEventBus } from 'types/manager.types';
import { ProviderErrorsEnum } from 'types/provider.types';
import { createModalElement } from 'utils/createModalElement';
import { WalletConnectOptionalMethodsEnum, WalletConnectV2Provider } from 'utils/walletconnect/__sdkWalletconnectProvider';
import { WalletConnectEventsEnum, WalletConnectV2Error, WalletConnectConfig, IWalletConnectModalData } from './types';

const dappMethods: string[] = [WalletConnectOptionalMethodsEnum.CANCEL_ACTION, WalletConnectOptionalMethodsEnum.SIGN_LOGIN_TOKEN];
...
export class WalletConnectProviderStrategy {
  ...
  private manager: WalletConnectStateManager<IEventBus<IWalletConnectModalData>> | null = null;
  private approval: (() => Promise<SessionTypes.Struct>) | null = null;
  private unsubscribeEvents: (() => void) | null = null;
  private _login: ((options?: { approval?: () => Promise<SessionTypes.Struct>; token?: string }) => Promise<IProviderAccount | null>) | null = null;
  ...
  public createProvider = async (): Promise<IProvider> => {
    ...
    if (!this.provider && this.config) {
      const { walletConnectProvider, dappMethods } = await this.createWalletConnectProvider(this.config);

      // Bind in order to break reference
      this._login = walletConnectProvider.login.bind(walletConnectProvider);
      this._signTransactions = walletConnectProvider.signTransactions.bind(walletConnectProvider);
      this._signMessage = walletConnectProvider.signMessage.bind(walletConnectProvider);
  ...
  private createEventBus = async () => {
    ...
    const modalElement = await createModalElement<WalletConnectModal>('wallet-connect-modal');
  ...
  private createWalletConnectProvider = async (config: WalletConnectConfig) => {
    ...
    try {
      const { walletConnectV2ProjectId, walletConnectV2Options = {}, walletConnectV2RelayAddress = '' } = config;
      const walletConnectProvider = new WalletConnectV2Provider(providerHandlers, chainId, walletConnectV2RelayAddress, walletConnectV2ProjectId, walletConnectV2Options);
  ...
  private login = async (options?: {
    ...
    if (!this.provider || !this.manager) {
      throw new Error('Provider or manager is not initialized. Call createProvider first.');
    }
  ...
  private sendCustomRequest = async ({ action, method }: { action: OptionalOperation; method: WalletConnectOptionalMethodsEnum }) => {
    if (!this.provider) {
      throw new Error(ProviderErrorsEnum.notInitialized);
    }
